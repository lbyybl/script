...
filter_fdr <- function(file,name){
  ...
  unchange <- data %>%
    dplyr::filter(FDR >= 0.05 | (FDR < 0.05 & abs(Fold) < log(2))) %>%
    dplyr::select(seqnames,start,end)
  #unchange[,.(seqnames,start,end)]
  down <- data %>% 
    dplyr::filter(FDR <0.05 & Fold > log(2)) %>%
    dplyr::select(seqnames,start,end)
  up <- data %>% 
    dplyr::filter(FDR < 0.05 & Fold < -log(2)) %>%
    dplyr::select(seqnames,start,end)
...
filter_fdr('Rdox1h_deseq2.txt','Rdox1h')

#----各个文见之间进行overlap 并将空的去除???
dir.create('overlap')
dir.create('heatmap')
#dir.create('anno')
