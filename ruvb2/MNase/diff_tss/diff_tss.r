...
diffbind <- function(file,name){
  ...
  dba.plotMA(dbObj,bXY=F,xrange = c(0,4),yrange = c(-4,4),bSmooth = T,fold = log(2))
  dev.off()
  pdf(paste0(name,'scatter_plot.pdf'),width = 5,height = 5.5)
  dba.plotMA(dbObj,bXY=T,xrange = c(-1,5),yrange = c(-1,5),bSmooth = T,fold = log(2))
  dev.off()
...
setwd('/WORK/lbyybl/WH/rvb/cor_uniq/MNase/graph/diff_tss')
