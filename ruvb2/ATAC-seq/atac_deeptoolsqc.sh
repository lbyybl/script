...
cd ${dir}
	...
	cat ${dir}/rmchrM/peak/*.narrowPeak > ${dir}/${prefix}_union_merge.narrowPeak
	...
	multiBigwigSummary BED-file -b ${dir}/rmchrM/bw/*.bw --BED ${dir}/${prefix}_union_merge.narrowPeak --smartLabels -out ${dir}/${prefix}_summary_peak.npz --outRawCounts ${dir}/${prefix}_summary_peak.tab -p 10 -bl /DATA/work/lbyybl/genomes/mm10/mm10.blacklist.bed
	...
	multiBigwigSummary bins -b ${dir}/rmchrM/bw/*.bw --binSize 10000 --smartLabels -out ${dir}/${prefix}_summary_bin.npz --outRawCounts ${dir}/${prefix}_summary_bin.tab -p 10 -bl /DATA/work/lbyybl/genomes/mm10/mm10.blacklist.bed
	...
	plotFingerprint --skipZeros -bl /DATA/work/lbyybl/genomes/mm10/mm10.blacklist.bed -b ${dir}/rmchrM/*.bam --smartLabels -T "Fingerprints of different samples" --plotFile ${dir}/${prefix}_fingerprints.pdf --outRawCounts ${dir}/${prefix}_fingerprints.txt \
	--outQualityMetrics ${dir}/${prefix}_fingerprints.metrics -p 10 1> ${dir}/${prefix}_fingerprints.log 2>&1
	
	#--- sequence deepth
	plotCoverage -b ${dir}/rmchrM/*.bam --plotFile ${dir}/${prefix}_coverage.pdf -n 1000000 --plotTitle "coverage" --outRawCounts ${dir}/${prefix}_coverage.tab -e 1> ${dir}/${prefix}_coverage.log 2>&1
