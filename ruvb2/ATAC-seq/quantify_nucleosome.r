#---- MNase profile 
setwd('/WORK/lbyybl/WH/rvb/ATAC/sample20191014/graph/nuclear_position/danpos')
...
tss_profile <- fread('Ruvbl_tss_rank_mergemono.gz',skip = 1,stringsAsFactors = F)
names(tss_profile)
h051 <- tss_profile[,7:406]
h052 <- tss_profile[,407:806]
h11 <- tss_profile[,807:1206]
h12 <- tss_profile[,1207:1606]
dox1 <- tss_profile[,1607:2006]
dox2 <- tss_profile[,2007:2406]
h051_mean<- trans_df(h051,'h051')
h052_mean<- trans_df(h052,'h052')
h11_mean<- trans_df(h11,'h11')
h12_mean<- trans_df(h12,'h12')
dox1_mean <-trans_df(dox1,'dox1')
dox2_mean <- trans_df(dox2,'dox2')
plot(dox1_mean$coor,dox1_mean$value,type='l')

merge <- rbind(dox1_mean,h051_mean)
merge <- rbind(merge,h11_mean)
merge <- rbind(merge,h052_mean)
merge <- rbind(merge,dox2_mean)
merge <- rbind(merge,h12_mean)
...
ggsave('all_tss.pdf',width=7,height = 3)
