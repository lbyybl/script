...
{
	echo ""
	echo -e "Usage: $0 [options] -f <Reads1_file> -r <Reads2_file> -o <Output_dir> -m <Primer3_file> -p <Prefix> -s <self_re_lig>"
	echo ""
...
while getopts "f:r:o:m:p:s:h" optionName
do
	case $optionName in
...
echo ${selfrelig}
if [[ $selfrelig = "" ]]; then
	echo " -r the selfrelig file is needed "
	exit 1
elif [[ ! -f $selfrelig ]]; then
	echo "$selfrelig:   is not found"
	exit 2
fi
...
#---rm self re, duplicate, dumped
	...
	cd mapping/
	pwd
	mkdir -p rm_dup rm_self_re rm_dumped bigwig
	#--- rm duplicate		
		samtools sort -@ 40 ${prefix}_local.sam >> ${prefix}_local.bam
		sambamba markdup -r -t 40 ${prefix}_local.bam rm_dup/${prefix}_local.rmdup.bam
	...
	#--- get bigwig
		...
		bamCoverage -b rm_dumped/${prefix}_local.rmdump.bam -o bigwig/${prefix}_rmdup.bw --normalizeUsing RPKM --blackListFileName bigwig/blacklist
		bamCoverage --bam rm_dumped/${prefix}_local.rmdump.bam -o bigwig/${prefix}_rpgc.bw --binSize 10 --normalizeUsing RPGC --effectiveGenomeSize 2652783500 --blackListFileName bigwig/blacklist
#--- call peak
	mkdir -p peak
...
#--- stastics
	...
	#---- stastic file
	echo -e "sample_name,total_resds,select enzyme site,select 3' primer, mapping reads,rm duplicate,rm self-ligatin and religation,rm dumped" >> stastic.csv
	echo -e "${prefix},${total_resds},${sel_enz},${sel_3p},${mapping},${rmdup},${rm_re_self},${rmdump}" >> stastic.csv
