...
if [[ ! -d ${outputdir}/QC ]]; then
	...
	fastq_prefix2=${fastq_name%%.*}
	if [[ ! -f ${outputdir}/QC/${fastq_prefix}_fastqc.html && ! -f ${outputdir}/QC/${fastq_prefix2}_fastqc.html ]]; then
		mkdir -p ${outputdir}/QC
		#--- 1. 质控
		fastqc ${fastq1} ${fastq2} -o ${outputdir}/QC
	fi
...
elif [[ ! -f ${outputdir}/cutadapt/${Prefix}_1.fq ]]; then
	mkdir -p ${outputdir}/cutadapt
	cutadapt -a ${forward_adapt} -a "C{150}" -A ${reverse_adapt} -G "G{150}" -q 15,15 --overlap 1 -m ${min_length} -o ${outputdir}/cutadapt/${Prefix}_1.fq -p ${outputdir}/cutadapt/${Prefix}_2.fq ${fastq1} ${fastq2}
	fastqc ${outputdir}/cutadapt/${Prefix}_1.fq ${outputdir}/cutadapt/${Prefix}_2.fq -o ${outputdir}/cutadapt
fi
...
# bamCoverage --bam ${outputdir}/mapping/${Prefix}_rmdup.bam -o ${outputdir}/bigwig/${Prefix}_rpgc.bw --binSize 10 --normalizeUsing RPGC --effectiveGenomeSize 2652783500 --blackListFileName ${outputdir}/bigwig/${Prefix}_blacklist
# #--- 5. call peak
...
if [[ ! -f ${outputdir}/${Prefix}_stastic_all.csv ]]; then
	...
	Rscript /home/boyuanli/bashscript/bin/chip_seq/stastic_reads_info.r -b ${outputdir}/mapping/${Prefix}_mapping_format.stat -s ${outputdir}/${Prefix}_stastic.csv -o ${outputdir}/${Prefix}_stastic_all.csv
