...
#samtools view -hF4 $bam_file | grep -v "XS:" | samtools view -b - >> ${output_dir}/${prefix}tmp_uniqe.bam

sambamba view -h --nthreads 10 -f bam -F "[XS] == null and not unmapped and paired and proper_pair and not duplicate" $bam_file >> ${output_dir}/${prefix}_uniqe.bam
samtools index ${output_dir}/${prefix}_uniqe.bam
samtools flagstat ${output_dir}/${prefix}_uniqe.bam >> ${output_dir}/${prefix}_uniqe.flag
