...
awk -v Genomenane="$Genomenane" '{printf "%i|%s|%s:%i-%i\n",$4,Genomenane,$1,$2,$3}'  $Input_bedfile >> ${prefix}_header

chrname=$(cut -f 1 <(egrep -v "chrM" $Genomesizefile) | xargs | sed 's/ /\\n/g')
echo -e "$chrname" | parallel -j1 egrep "{}:" ${prefix}_header '>>' ${prefix}_{}_header
...
# bed(no chrM/chrMT);
echo -e "$chrname" | parallel -j1 egrep "{}" $Input_bedfile '>>' ${prefix}_no_chrM.bed
...
egrep -v "$chrM_correspording_binnumber|$chrM_correspording_binnumber2" $Input_matrixfile >> ${prefix}_no_chrM.matrix

# mergeed eg1 file
echo -e "$chrname" | parallel -j1 cat ${filename%.*}_{}_dense.addedHeaders.zScore.compartments >> ${filename%.*}_merge_zSxore.compartments

##########################################################################################################################################################
###got resort file
## after this step, you will got two files, one is the ${prefix}_no_chrM_resort.bed,another is the sparse matrix that have resort :${prefix}_no_chrM_sparse.matrix
## and you can use it to draw heatmap (use python or Hicplotter)
#################################################################################################################################################
matrixhic-resorter ${filename%.*}_merge_zSxore.compartments \
${prefix}_no_chrM.bed ${prefix}_no_chrM.matrix ${prefix}_no_chrM_sparse.matrix 
