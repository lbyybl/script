...
echo "enter input dir....."
	...
	ls | xargs -n 2 | parallel -C " " -j ${cup2} "hisat2 -x ${index} -1 {1} -2 {2} -S ${outdir}/mapping/{=1 s/_1\..*$//g;s/_R1\..*$//g;s/_1_.*$//g;s/_1_.*$//g;s/_R1.*$//g=}.sam --dta --dta-cufflinks 2>> ${outdir}/mapping/{=1 s/_1\..*$//g;s/_R1\..*$//g;s/_1_.*$//g;s/_1_.*$//g;s/_R1.*$//g=}.log "
	cd ${outdir}/mapping 
	ls *.sam | parallel -j ${cup2} "samtools view -bS {} | samtools sort - -o {.}_sort.bam; samtools index {.}_sort.bam;rm {}"
	ls *.bam | parallel -j ${cup2} "sambamba markdup -r -t 10 {} {.}_rmdup.bam"
	mkdir -p ${outdir}/stringtie 
	ls *_rmdup.bam | parallel -j ${cup2} "stringtie -e -B -p $CUP_num -G ${gff_file} -o ${outdir}/stringtie/{.}.gtf -l {.} {}"
	cd ${outdir}/stringtie
	...
	cat ${outdir}/mapping/namelist.txt | parallel "stringtie -e -B -p $CUP_num -G ${outdir}/stringtie/stringtie_merged.gtf -o ${outdir}/ballgown/{}/{}.gtf ${outdir}/mapping/{}_sort_rmdup.bam"
